...
** Last update Tue Apr 12 11:28:15 2016 alies_a
*/
...
char	*dump_next(char **next, int *next_size, int x)
{
  ...
  size = *next_size - x;
  if ((new_next = malloc(sizeof(char) * (size + 1))) == NULL)
    return (NULL);
...
char	*get_line(char **next, int *next_size, char del)
{
  int	x;
...
char		*get_next_line(const int fd)
{
  ...
  if (fd < 0 || fd > 99)
    return (NULL);
  while ((res = get_line(&next, &size, '\0')) == NULL)
    {
